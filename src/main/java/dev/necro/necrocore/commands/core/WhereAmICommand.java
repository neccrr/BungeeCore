    ...
    public void whereAmICommand(final @NonNull CommandSender sender) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (player.getServer().getInfo() != null) {
                player.sendMessage(new TextComponent(plugin.getMainConfigManager().getPREFIX() + StringUtils.colorize("&bYou are currently connected to &6" + player.getServer().getInfo().getName())));
            } else {
                player.sendMessage(new TextComponent(plugin.getMainConfigManager().getPREFIX() + StringUtils.colorize("&cCannot get the server information! Please try again later.")));
            }
